...
const openai_1 = __importDefault(require("openai")); // Use OpenAI SDK
const winston_1 = __importDefault(require("winston"));
const dedicatedPlannerPrompt_1 = require("./conversation/prompts/dedicatedPlannerPrompt"); // New prompt
...
class PlannerService extends events_1.EventEmitter {
    client; // OpenAI client will be initialized in constructor
    model;
    maxTokens;
    toolConfigManager;
    constructor(openAIApiKey, // Changed from groqApiKey to openAIApiKey
    model, maxTokens, toolConfigManager) {
        super();
        this.client = new openai_1.default({ apiKey: openAIApiKey }); // Initialize OpenAI client
        this.model = model;
     ...
     */
    async generatePlan(userInput, identifiedToolCalls, sessionId, clientMessageId // Added to associate planner status with original message
    ) {
