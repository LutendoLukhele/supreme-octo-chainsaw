...
// --- Core Dependencies ---
const config_1 = require("./config"); // Ensure GROQ_API_KEY, TOOL_CONFIG_PATH, NANGO_CONNECTION_ID, etc.
// --- Services ---
...
const ToolConfigManager_1 = require("./services/tool/ToolConfigManager"); // Import ToolConfigManager
const action_launcher_service_1 = require("./action-launcher.service"); // Import ActionLauncherService
const BeatEngine_1 = require("./BeatEngine"); // Added BeatEngine import
const ScratchPadStore_1 = require("./services/scratch/ScratchPadStore"); // Added ScratchPadStore & ScratchEntry import
// --- External SDKs ---
const groq_sdk_1 = __importDefault(require("groq-sdk"));
const chat_hanndler_1 = require("./services/chat_hanndler");
...
if (!config_1.CONFIG.OPEN_AI_API_KEY) { // Good to check this one too if it's critical
...
const toolConfigManager = new ToolConfigManager_1.ToolConfigManager(config_1.CONFIG.TOOL_CONFIG_PATH); // Initialize Tool Manager FIRST
...
const conversationConfig = {
    groqApiKey: config_1.CONFIG.GROQ_API_KEY, // Now guaranteed to be a string due to the check above
    // openAIApiKey: CONFIG.OPEN_AI_API_KEY, // If ConversationConfig needs this, add it similarly
    model: config_1.CONFIG.MODEL_NAME, maxTokens: config_1.CONFIG.MAX_TOKENS,
...
const streamManager = new StreamManager_1.StreamManager({ logger, chunkSize: config_1.CONFIG.STREAM_CHUNK_SIZE || 512 });
...
scratchPadService.on('scratchpadSeeded', (sessionId, entries) => {
...
// --- State Maps ---
const sessionUserIdMap = new Map(); // Map<sessionId, userId>
// --- Event Handlers ---
followUpService.on('follow_up_generated', ({ userId, sessionId, messageId, toolCallId, fullResponse }) => {
    ...
    if (!userId) {
        logger.error('Follow-up event missing userId', { sessionId });
        return;
    }
    ...
    (0, chat_hanndler_1.recordFollowUpResponse)(userId, sessionId, messageId, fullResponse, toolCallId) // userId FIRST
...
followUpService.on('send_chunk', (sessionId, chunk) => {
    if (!chunk || typeof chunk !== 'object' || !sessionId) {
        logger.error("Invalid send_chunk data", { sessionId });
        return;
    }
...
wss.on('connection', async (ws, req) => {
    ...
    if (!sessionId || sessionId.length < 5) {
        logger.error('Connection rejected: Invalid sessionId');
        ws.close(1008);
        return;
    }
    ...
    ws.on('message', async (message) => {
        ...
        try {
            ...
            try {
                ...
                logger.error("Failed WS JSON parse", { sessionId, error: e.message });
                return;
            }
            ...
            if (data.type === 'init') {
                if (data.userId && typeof data.userId === 'string') {
                    ...
                    kickoffBeats.forEach(b => streamManager.sendChunk(sessionId, { type: 'beat', content: b }));
                    logger.info(`Sent ${kickoffBeats.length} session-start beats`, { sessionId, userId: currentUserId });
                }
                else {
                    logger.error('Init failed: userId missing/invalid', { sessionId });
                    ws.close(1008);
                    return;
                }
                return;
            ...
            if (!isInitialized || !currentUserId) {
                logger.warn('Msg before init', { sessionId });
                ws.close(1008);
                return;
            }
            ...
            switch (data.type) {
                case 'update_parameter':
                    if (data.content && typeof data.content === 'object') {
                        ...
                        if (updatedAction)
                            streamManager.sendChunk(sessionId, { type: 'parameter_updated', content: updatedAction });
                        else
                            logger.warn('Parameter update failed', { sessionId, payload });
                    }
                    else {
                        logger.warn('Invalid update_parameter format', { sessionId });
                    }
                    return;
                case 'execute_action':
                    if (data.content && typeof data.content === 'object') {
                        ...
                        try {
                            ...
                            if (executedAction.status === 'completed' && executedAction.result) {
                                ...
                                if (originalMessageId) {
                                    ...
                                    logger.warn("Cannot trigger follow-up, original messageId link missing.", { actionId: payload.actionId });
                                }
                            }
                        }
                        catch (execError) {
                        ...
                        logger.warn('Invalid execute_action format', { sessionId });
                    }
                    return;
            }
            ...
            if (data.content && typeof data.content === 'string') {
                ...
                messageId = await (0, chat_hanndler_1.handleChatMessage)(currentUserId, sessionId, data.content);
                logger.info('User message/placeholder recorded', { userId: currentUserId, sessionId, messageId });
                // 2. Process Message (Single LLM Call)
                const aiResponse = await conversationService.processMessage(data.content, sessionId);
                ...
                await (0, chat_hanndler_1.recordAiResponseAndToolCalls)(currentUserId, sessionId, messageId, aiResponse);
                ...
                if (aiResponse.toolCalls && aiResponse.toolCalls.length > 0) {
                    ...
                    preBeats.forEach(b => streamManager.sendChunk(sessionId, { type: 'beat', content: b }));
                }
                // --- DECISION & EXECUTION ---
                ...
                if (toolCalls && toolCalls.length > 0) {
                    ...
                    if (isParamRequest) {
                        ...
                        try {
                            ...
                            if (!intended_tool_name || !missing_params || !clarification_question)
                                throw new Error("Invalid request_missing_parameters args");
                            // SCENARIO A: LLM called request_missing_parameters
                            const response = actionLauncherService.createLauncherResponseForParameterRequest(intended_tool_name, missing_params, clarification_question, sessionId, messageId // Pass original messageId
                            );
                        ...
                        for (const toolCall of regularCalls) {
                            ...
                            if (allMissingParams.length > 0) {
                                ...
                                const messagesToClient = await actionLauncherService.initiateServerSideParameterCollection(sessionId, currentUserId, messageId, toolName, allMissingParams, parsedArgs);
                                messagesToClient.forEach((msg) => streamManager.sendChunk(sessionId, msg));
                                allToolCallsProceededWithoutNeedingParams = false;
                            }
                            else {
                ...
                logger.warn('Received initialized message with unknown type/content', { sessionId });
            }
        }
        catch (error) {
            ...
            try {
                streamManager.sendChunk(sessionId, { type: 'error', content: `Server error: ${error.message || 'Unknown error'}`, messageId });
            }
            catch (sendError) {
                logger.error("Failed sending error chunk", { sessionId });
            }
        }
    }); // End ws.on('message')
...
async function validateToolCalls(toolCalls, configManager // Pass instance
) {
    const validToolCalls = [];
    const invalidToolCallsInfo = [];
    for (const toolCall of toolCalls) {
        ...
        try {
            ...
            parseError = true;
            logger.warn("Arg parse failed in validation", { toolCallId: toolCall.id });
        }
...
async function handleToolCalls(validToolCalls, userId, sessionId, messageId) {
    logger.info(`Executing ${validToolCalls.length} validated direct tool calls`, { userId, sessionId, messageId });
    const nangoConnectionId = config_1.CONFIG.CONNECTION_ID; // Get Nango ID from config
    ...
    for (const toolCall of validToolCalls) {
        ...
        try {
            ...
            try {
                ...
                logger.warn("Failed to parse tool arguments for scratchpad filters", { toolCallId: toolCall.id, args: toolCall.function.arguments });
            }
            // Execute Tool
            ...
            await (0, chat_hanndler_1.updateToolCallResult)(userId, sessionId, messageId, toolCall.id, result);
            streamManager.sendChunk(sessionId, {
                type: 'tool_result',
                messageId: messageId,
            ...
            const errorResult = { status: 'failed', toolName: toolCall.function.name, data: null, error: toolError.message };
            try {
                await (0, chat_hanndler_1.updateToolCallResult)(userId, sessionId, messageId, toolCall.id, errorResult);
            }
            catch (dbError) { /* log */ }
...
process.on('SIGTERM', () => {
    ...
    const checkAndExit = () => { if (httpServerClosed && wsServerClosed) {
        clearTimeout(shutdownTimeout);
        logger.info('Shutdown complete.');
        process.exit(0);
    } };
    logger.info('Closing WebSocket server...');
    wss.close((wsErr) => { if (wsErr)
        logger.error('WSS close error:', wsErr); wss.clients.forEach((wsClient) => wsClient.terminate()); sessionUserIdMap.clear(); actionLauncherService.clearAllActions(); wsServerClosed = true; checkAndExit(); }); // Clear actions map
    logger.info('Closing HTTP server...');
    server.close((httpErr) => { if (httpErr)
        logger.error('HTTP close error:', httpErr); httpServerClosed = true; checkAndExit(); });
});
