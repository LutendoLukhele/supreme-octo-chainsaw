// src/utils/content-transformer.ts

import { marked } from 'marked';
import { InterpretiveResponse, Segment } from '../models/interpretive.model';

export interface TransformedContent {
    title: string;
    topics: string[];
    hero: InterpretiveResponse['hero'];
    segments: InterpretiveResponse['segments'];
    sources: InterpretiveResponse['sources'];
    markdown?: string;
    html?: string;
    plaintext?: string;
    json?: string;
...
export class ContentTransformer {
    public async transform(content: InterpretiveResponse, format: string): Promise<TransformedContent> {
        const base: TransformedContent = {
    ...
    private toMarkdown(content: InterpretiveResponse): string {
        let md = '';
    ...
    private toHTML(content: InterpretiveResponse): string {
        const markdown = this.toMarkdown(content);
        return marked.parse(markdown) as string;
    }

    private toPlaintext(content: InterpretiveResponse): string {
        let text = '';
    ...
    private segmentToPlaintext(segment: Segment): string {
        switch (segment.type) {
            case 'text':
                return `${segment.text}\n\n`;
            case 'insight': {
    ...
    private extractTopics(content: InterpretiveResponse): string[] {
        const topics: string[] = [];
