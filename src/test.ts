...
const nango = new Nango({ secretKey: '7addd614-fda8-48a2-9c79-5443fda50a84' });

// Implementation similar to your NangoService.triggerSalesforceAction
async function triggerSalesforceAction(
  operation: string,
  entityType: string,
  actionOptions: Record<string, any>
): Promise<any> {
  let actionName: string;
  let payload: Record<string, any> = { operation, entityType, ...actionOptions };
...
async function fetchAccountIdForTesting(): Promise<string | null> {
  try {
    // Fetch a test account
    const accounts = await triggerSalesforceAction(
      'fetch',
      'Account',
      {
        filters: {
...
async function testEnhancedUpdates() {
  ...
  try {
    ...
    const updateResult = await triggerSalesforceAction(
      'update',
      'Account',
      {
    ...
    const batchUpdateResult = await triggerSalesforceAction(
      'update',
      'Account',
      {
        filters: {
    ...
    const complexUpdateResult = await triggerSalesforceAction(
      'update',
      'Account',
      {
        filters: {
    ...
    const allOrNothingResult = await triggerSalesforceAction(
      'update',
      'Account',
      {
        filters: {
