...
scratchPadService.on('scratchpadSeeded', (sessionId: string, entries: Record<string, ScratchEntry>) => {
  if (streamManager.hasConnection(sessionId)) { // Check if client is still connected
    logger.info(`Sending scratchpad_update via WebSocket for session ${sessionId}. Entry count: ${Object.keys(entries).length}`);
    streamManager.sendChunk(sessionId, {
      type: 'scratchpad_update',
      content: entries, // The full map of entries
    });
  } else {
    ...
    try {
      ...
      if (data.type === 'init') {
         if (data.userId && typeof data.userId === 'string') {
             ...
             sessionUserIdMap.set(sessionId, data.currentUserId);
             isInitialized = true;
